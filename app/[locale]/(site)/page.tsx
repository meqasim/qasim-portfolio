    ...
    <main className="mx-auto max-w-6xl px-4 space-y-16">
        ...
        <div className="grid items-center gap-8 md:grid-cols-2">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a className="rounded-2xl bg-[var(--primary)] px-4 py-2 text-sm font-medium text-black hover:opacity-90" href={site.whatsapp} target="_blank" rel="noreferrer">{tHero("ctaHire")}</a>
              <a className="rounded-2xl border border-[var(--border)] px-4 py-2 text-sm transition-colors hover:border-[var(--primary)]" href={`mailto:${site.email}`}>{tHero("ctaEmail")}</a>
              <Link className="rounded-2xl border border-[var(--border)] px-4 py-2 text-sm transition-colors hover:border-[var(--primary)]" href={href("/resume")}>Resume</Link>
              <Link className="rounded-2xl border border-[var(--border)] px-4 py-2 text-sm transition-colors hover:border-[var(--primary)]" href={href("/about")}>About</Link>
            </div>
          </div>
          <div className="relative mx-auto max-w-[380px] group">
            ...
            <Image
              ...
              height={1536}
              priority
              className="rounded-[28px] border border-[var(--border)] shadow-2xl transition-colors group-hover:border-[var(--primary)]"
            />
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <ul className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {services.map(s => (
            <li key={s.title} className="rounded-2xl border border-[var(--border)] p-5 shadow-sm transition-colors hover:border-[var(--primary)]">
